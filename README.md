# medical-fraud-detection-examples

Examples for medical fraud discussion.
...
3 - Explore - If you are more into Python, a good place to start is `jupyter lab` from the root of the project, and look in the `./notebooks` directory. If GIS is more your schtick, open the project `./arcgis/medical-fraud-detection-examples.aprx`.
...
- `make env` - builds the Conda environment with all the name and dependencies from `environment_dev.yml` and installs the local project package `med_fraud` using the command `python -m pip install -e ./src/src/med_fraud` so you can easily test against the package as you are developing it.
